...
const CategoriesSelector = (p: Props) => {
    ...
    return <LoadedZone loading={uiContext.categories.loading} error={uiContext.categories.error} containerStyle={{ marginLeft: '1rem' }}>
            { uiContext.categories.data && uiContext.categories.data.map((cat: any, idx: number) => <FormControlLabel key={idx} control={
