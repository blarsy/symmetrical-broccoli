...
BEGIN
	...
	SELECT r.*
	  ...
	  AND(
		 ...
		 OR
		 (select sALTER TABLE IF EXISTS sb.accounts
    ADD COLUMN willing_to_contribute boolean NOT NULL DEFAULT false;
...
END;
$body$
LANGUAGE 'plpgsql'; b.geodistance(suggested_resources.reference_location_latitude, suggested_resources.reference_location_longitude, l.latitude, l.longitude) <= suggested_resources.distance_to_reference_location)
	  )
	  AND
	  (search_term = '' OR 
		(r.title ILIKE '%' || search_term || '%' OR 
