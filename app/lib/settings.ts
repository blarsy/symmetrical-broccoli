import Constants from 'expo-constants'

export const apiUrl = Constants.expoConfig!.extra!.appSettings.apiUrl
export const graphQlApiUrl = Constants.expoConfig!.extra!.appSettings.graphQlApiUrl
export const subscriptionsUrl = Constants.expoConfig!.extra!.appSettings.subscriptionsUrl
export const linksUrl = Constants.expoConfig!.extra!.appSettings.linksUrl
export const cloudinaryCloud = Constants.expoConfig!.extra!.appSettings.cloudinaryCloud
export const cloudinaryUploadPreset = Constants.expoConfig!.extra!.appSettings.cloudinaryUploadPreset
export const cloudinaryRestUrl = Constants.expoConfig!.extra!.appSettings.cloudinaryRestUrl
export const clientVersion = Constants.expoConfig!.extra!.appSettings.clientVersion
export const googleMapsApiKey = Constants.expoConfig!.extra!.appSettings.googleMapsApiKey
export const googleAuthWebClienttId = Constants.expoConfig!.extra!.appSettings.googleAuthWebClienttId
export const diagnostic = Constants.expoConfig!.extra!.appSettings.diagnostic

